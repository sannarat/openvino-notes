...
class UpdateNoteUseCaseTest {
    ...
    @Test
    fun `invoke should return failure when duplicate title exists in same folder`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val folderId = "folder_1"

        val existingNote = Note(
            userId = "u1", id = "note_1", title = "Meeting",
            folderId = folderId, createdAt = now, updatedAt = now
        )
        val noteToUpdate = Note(
            userId = "u1", id = "note_2", title = "  meeting  ",
            folderId = folderId, createdAt = now, updatedAt = now
        )
    ...
    @Test
    fun `invoke should succeed when updating same note with same title`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val note = Note(userId = "u1", id = "1", title = "Original", folderId = "A", createdAt = now, updatedAt = now)
    ...
    @Test
    fun `invoke should succeed when same title is in different folder`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val existingNote = Note(userId = "u1", id = "1", title = "Same", folderId = "Folder_A", createdAt = now, updatedAt = now)
        val noteToUpdate = Note(userId = "u1", id = "2", title = "Same", folderId = "Folder_B", createdAt = now, updatedAt = now)
