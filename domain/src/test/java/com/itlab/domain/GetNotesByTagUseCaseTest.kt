...
class GetNotesByTagUseCaseTest {
    ...
    @Test
    fun `invoke should return all notes when tag is blank`() = runBlocking {
    ...
    @Test
    fun `invoke should filter notes by tag case insensitively`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val notes = listOf(
            Note(userId = "u1", title = "Work Note", tags = setOf("Work", "Urgent"), createdAt = now, updatedAt = now),
    ...
    @Test
    fun `invoke should return empty list when no notes match tag`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val notes = listOf(
