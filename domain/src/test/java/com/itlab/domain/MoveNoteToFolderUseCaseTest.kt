...
import com.itlab.domain.model.NoteFolder // Предположим, такой класс есть
...
class MoveNoteToFolderUseCaseTest {
    ...
    @Test
    fun `invoke should update note folder when both exist`() = runBlocking {
    ...
    @Test
    fun `invoke should return failure when folder not found`() = runBlocking {
    ...
    @Test
    fun `invoke should return failure when note not found`() = runBlocking {
        val folderId = "folder_1"
