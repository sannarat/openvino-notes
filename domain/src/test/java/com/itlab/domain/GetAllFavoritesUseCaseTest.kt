...
class GetAllFavoritesUseCaseTest {
    ...
    @Test
    fun `invoke should filter only favorite notes`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
        val note1 = Note(userId = "u1", id = "1", title = "Fav", isFavorite = true, createdAt = now, updatedAt = now)
        val note2 = Note(userId = "u1", id = "2", title = "Not Fav", isFavorite = false, createdAt = now, updatedAt = now)
    ...
    @Test
    fun `invoke should return empty list when no favorites exist`() = runBlocking {
        val now = Instant.fromEpochMilliseconds(System.currentTimeMillis())
